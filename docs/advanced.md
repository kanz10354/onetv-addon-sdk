...
The `catalog` resource in OneTV addons can be used to:
...
This is not a requirement though, as if we don't set `options` OneTV will request `skip` once it comes to the end of your catalog.
...
Cinemeta is the primary addon that OneTV uses to show Movie, Series and Anime items. Other addons can choose to create their own catalogs of items or respond with streams to the Cinemeta items.
...
needle.get('https://api.example.com/meta/' + itemType + '/' + itemImdbId + '.json', function(err, resp, body) {
...
This example does not use the OneTV Addon SDK, it uses Node.js and Express to serve replies.
...
- IMDB Lists - community addon example
- IMDB Watchlist - community addon example
- Jackett Addon for OneTV (community built)

Another use case for passing user data through the Addon Repository URL is creating proxy addons. This case presumes that the id of a different addon is sent in the Addon Repository URL, then the proxy addon connects to the addon of which the id it got, requests streams, passes the stream url to some API (for example Real Debrid, Premiumize, etc) to get a different streaming url that it then responds with for OneTV.
...
An "Install Addon" button should be available on the page that will use the `onetv://` protocol, for example, if your Addon Repository URL is `https://my.addon.com/some-user-data/manifest.json`, the "Install Addon" button should point to `onetv://my.addon.com/some-user-data/manifest.json`. (the `onetv://` protocol links will open or focus the OneTV app with a prompt to install the addon)


## Using Deep Links in Addons

OneTV supports [deep links](./deep-links.md), such links can also be used in addons to link internally to OneTV.
...
// movie, that if clicked, will redirect OneTV to the
// Board page
builder.defineStreamHandler(function(args) {
  return new Promise(function(resolve, reject) {
    if (args.type === 'movie' && args.id === 'tt1254207') {
      // serve one stream for big buck bunny
      const stream = { externalUrl: 'onetv:///board' }
      resolve({ streams: [stream] })
    } else {
...
OneTV addons use a HTTP server to handle requests and responses, this means that other addons can also request their responses.

This can be useful for many reasons. Community addons like IMDB Watchlist demonstrate this pattern by proxying IMDB Lists addon to get the IMDB List for a particular IMDB user.
...
A simplistic version of doing this can use [needle](https://www.npmjs.com/package/needle) to request the HTML source and [cheerio](https://www.npmjs.com/package/cheerio) to start a jQuery instance to simplify getting the desired information.
