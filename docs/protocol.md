...
## OneTV Addon Protocol

**If you're creating an addon, we recommend you build it using our [addon-sdk](https://github.com/OneTV/onetv-addon-sdk), which will provide a convenient abstraction to the protocol, as well as an easy way of publishing your addons.**

The OneTV addon protocol defines a universal interface to describe multimedia content. It can describe catalogs, detailed metadata and streams related to multimedia content.

It is typically transported over HTTP or IPFS, and follows a paradigm similar to REST.

This allows OneTV or other similar applications to aggregate content seamlessly from different sources, for example YouTube, Twitch, iTunes, Netflix, DTube and others. It also allows developers to build such addons with minimal skill.
...
* `https://.../onetv/v1`: legacy transport
* `ipfs://.../manifest.json` or `ipns://.../manifest.json`: IPFS transport

For more details regarding the addon protocol, refer to the OneTV addon documentation
...
internally by OneTV's Cinemeta Addon.
...
Meta requests are done when OneTV opens a Details page for a movie (series, channel, etc.) The response is used to create the Details page, it can also handle "background", "logo", "releaseInfo" and other extended information about the movie / series / etc. As previously mentioned, the meta request does not need to be handled at all when using the IMDB
ID prefix, as that is handled internally by OneTV.


This addon is so simple that it can actually be hosted statically on GitHub pages!

See Example Static Addon in the OneTV documentation
...
* [Creating an addon with the NodeJS OneTV Addon SDK](https://github.com/OneTV/addon-helloworld)
* [Creating an addon with NodeJS and express](https://github.com/OneTV/addon-helloworld-express)
* Creating an addon with PHP - see examples documentation
* [Creating an addon with Python](https://github.com/onetv/addon-helloworld-python)
* [Creating an addon with Ruby](https://github.com/onetv/addon-helloworld-ruby)
* [Creating an addon with Go](https://github.com/onetv/addon-helloworld-go)
* [Creating an addon with C#](https://github.com/onetv/addon-helloworld-csharp)
* [Creating an addon with Java](https://github.com/onetv/addon-helloworld-java)
