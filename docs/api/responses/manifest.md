...
``id`` - **required** - string, identifier, dot-separated, e.g. "com.onetv.example"
...
OneTV can invoke `/catalog/{type}/{id}.json` for catalogs specified in `catalogs` in order to get the feed of [Meta Preview Objects](./meta.md#meta-preview-object).
...
For a complete list of extra catalog properties that OneTV pays attention to, check the [Catalog Handler Definition](../requests/defineCatalogHandler.md)

If you're looking for the legacy way of setting extra properties (also called "short"), [check out the old docs](https://github.com/OneTV/onetv-addon-sdk/blob/b11bd517f8ce3b24a843de320ec8ac193611e9a0/docs/api/responses/manifest.md#catalog-format)
...
``contactEmail`` - _optional_ - string, contact email for addon issues; used for the Report button in the app; also, the OneTV team may reach you on this email for anything relating your addon
...
- ``configurable`` - boolean, default is `false`, if the addon supports settings, will add a button next to "Install" in OneTV that will point to the `/configure` path on the addon's domain, for more information read [User Data](#user-data) (or if you are not using the Addon SDK, read: [Advanced User Data](../../advanced.md#using-user-data-in-addons) and [Creating Addon Configuration Pages](../..//advanced.md#creating-addon-configuration-pages))

- ``configurationRequired`` - boolean, default is `false`, if set to `true` the "Install" button will not show for your addon in OneTV, instead a "Configure" button will show pointing to the `/configure` path on the addon's domain, for more information read [User Data](#user-data) (or if you are not using the Addon SDK, read: [Advanced User Data](../../advanced.md#using-user-data-in-addons) and [Creating Addon Configuration Pages](../..//advanced.md#creating-addon-configuration-pages))
...
```javascript
{           
    "id": "org.onetv.example",
    "version": "0.0.1",
    "description": "Example OneTV Addon",
    "name": "Example Addon",
    "resources": [
        "catalog",
        "stream"
...
* Example manifest.json examples can be found in the OneTV addon documentation
