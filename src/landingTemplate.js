...
function landingTemplate(manifest) {
	const background = manifest.background || 'https://onetv.media/addon-background.jpg'
	const logo = manifest.logo || 'https://onetv.media/logo.png'
	...
	if ((manifest.config || []).length) {
		...
		if (options.length) {
			...
			const updateLink = () => {
				...
				installLink.href = 'onetv://' + window.location.host + '/' + encodeURIComponent(JSON.stringify(config)) + '/manifest.json'
			}
	...
	<head>
		<meta charset="utf-8">
		<title>${manifest.name} - OneTV Addon</title>
	...
	<body>
		...
		<script>
			...
			installLink.href = 'onetv://' + window.location.host + '/manifest.json'
		</script>
	</body>
